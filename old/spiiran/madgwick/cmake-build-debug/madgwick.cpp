...
void madgwick::set_koeff(float sampleFreq, float beta) {
    _beta = beta;
...
void madgwick::update(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
    ...
    recipNorm = inv_sqrt(_q0 * _q0 + _q1 * _q1 + _q2 * _q2 + _q3 * _q3);
    _q0 *= recipNorm;
    _q1 *= recipNorm;
    _q2 *= recipNorm;
    _q3 *= recipNorm;
